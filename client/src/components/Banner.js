...
const Banner = ({query,handleInputChange}) => {
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4 md:py-24 py-1 4'>
    <h1 className='text-5xl font-bold text-primary mb-3'>Find your <span className='text-blue'>new job </span>today</h1>
    <p className='text-lg text-black/70 mb-8'>Thousands of job in engineering and technology is waiting for you.</p>
    <form >
        <div className='flex justify-start md:flex-row flex-col md:gap-0 gap-4'>
            ...
            className='block flex-1 border-0 bg-transparent py-1.5 pl-8 text-gray-900 placeholder:text-gray-400 
            focus:right-0 sm:text-sm sm:leading-6' ></input>
            <CiLocationOn className=' absolute mt-2.5 ml-2 text-gray-400'/>
            </div> */}

            <button type='submit' className='bg-blue py-2 px-8 text-white md:rounded-s-none rounded ml-2'>Search</button>
        </div>
    </form>
    </div>
  )
